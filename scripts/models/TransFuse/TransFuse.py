...
from scripts.models.TransFuse.DeiT import deit_small_patch16_224 as deit
from scripts.models.TransFuse.DeiT import deit_base_patch16_224 as deit_base
from scripts.models.TransFuse.DeiT import deit_base_patch16_384 as deit_base_384
...
class TransFuse_S(nn.Module):
    def __init__(self, num_classes=1, drop_rate=0.2, normal_init=True, pretrained=False):
        ...
        if pretrained:
            self.resnet.load_state_dict(torch.load('scripts/models/TransFuse/resnet34-333f7ec4.pth'))
...
class TransFuse_L(nn.Module):
    def __init__(self, num_classes=1, drop_rate=0.2, normal_init=True, pretrained=False):
        ...
        if pretrained:
            self.resnet.load_state_dict(torch.load('resnet50-19c8e357.pth'))
        self.resnet.fc = nn.Identity()
        self.resnet.layer4 = nn.Identity()
...
def get_TransFuse():
    return SingleImageModelWrapper(TransFuse_S(pretrained=True))

if __name__ == '__main__':
    from scripts.utils import count_parameters
    import torch
    model = get_TransFuse().eval()
