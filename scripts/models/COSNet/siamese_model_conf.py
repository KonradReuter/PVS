...
class ResNet(nn.Module):
    ...
    def forward(self, x):
        ...
        x = F.interpolate(x, input_size, mode='bilinear')  #upsample to the size of input image, scale=8
...
class CoattentionModel(nn.Module):
    ...
    def load_params(self):
        saved_state_dict = torch.load('scripts/models/COSNet/deeplab_davis_12_0.pth', map_location=device)
    ...
    def forward(self, x): #注意input2 可以是多帧图像
        if args["save_attention_maps"]:
            x = x.permute(0, 2, 1, 3, 4).contiguous()
        assert x.shape[1] == 2, f"Number of frames per batch must be 2! Got {x.shape[1]}!"
        # input shape: batch x 2 x C x H x W
        input1 = x[:, 0, ...]
        input2 = x[:, 1, ...]
        ...
        x1 = F.interpolate(x1, input_size, mode='bilinear')  #upsample to the size of input image, scale=8
        x2 = F.interpolate(x2, input_size, mode='bilinear')  #upsample to the size of input image, scale=8
        #print("after upsample, tensor size:", x.size())
        #x1 = self.softmax(x1)
        #x2 = self.softmax(x2)
...
if __name__ == '__main__':
    input = torch.randn(3, 2, 3, 256, 256)
    model = CoattentionNet().eval() # calculate model.eval to allow batch norm to take batch size 1
    out = model(input)
    print(out.shape)
