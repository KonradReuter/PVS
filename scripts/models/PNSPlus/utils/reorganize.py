...
SUNSEG_test_list = [p.replace("\\", "/") for p in SUNSEG_test_list]
SUNSEG_train_list = [p.replace("\\", "/") for p in SUNSEG_train_list]
...
for SUN_path, SUNSEG_path in zip(SUN_list, SUNSEG_list):
    ...
    """
    SUN_path = SUN_path.replace("\\", "/")
    SUNSEG_path = SUNSEG_path.replace("\\", "/")
